...
<td>String</td>
<td>'local'</td>
<td>The current timezone to display the moment in. Posible values: 'local', 'utc' or a id added with <code>options.timezones</code> or by method <code>addTimezone(..)</code></td>
</tr>
</table>
...
Only show for format='`DMY`'

### Methods

    .setFormat( {format:'YMD', formatId:2, timeFormat:'12', timezoneId:'utc'} );
